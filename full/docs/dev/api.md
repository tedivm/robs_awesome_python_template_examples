...
The FastAPI application is defined in `full/www.py` and includes:

- **Automatic API documentation** at `/docs` (Swagger UI) and `/redoc` (ReDoc)
- **Static file serving** from `full/static/` via the `/static/` endpoint
...
Create a new route in `full/www.py`:
...
Static files are served from `full/static/`:
...
uvicorn full.www:app --reload --host 0.0.0.0 --port 8000
...
uvicorn full.www:app --host 0.0.0.0 --port 8000 --workers 4

# Or using gunicorn with uvicorn workers
gunicorn full.www:app -w 4 -k uvicorn.workers.UvicornWorker
```
