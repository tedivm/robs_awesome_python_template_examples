...
Templates are stored in the `full/templates/` directory:
...
The Jinja2 environment is configured in `full/services/jinja.py`:
...
from fastapi.templating import Jinja2Templates
from full.services.jinja import env, response_templates
...
def test_template_default_filter():
    """Test default filter."""
    template = env.from_string("{{ value|default('N/A') }}")
