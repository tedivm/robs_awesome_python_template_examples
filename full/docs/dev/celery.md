...
The Celery application is defined in `full/celery.py`. Unlike other components, Celery does NOT read configuration from the project's Settings class. Instead, Celery must be configured using environment variables.
...
```python
# In full/celery.py
from full.tasks import email, reports, cleanup
```
...
```bash
# Basic worker
celery -A full.celery worker --loglevel=info

# With concurrency limit
celery -A full.celery worker --loglevel=info --concurrency=4

# With specific queues
celery -A full.celery worker --loglevel=info -Q celery,emails
```
...
celery -A full.celery beat --loglevel=info

# Or combine worker and beat (NOT recommended for production)
celery -A full.celery worker --beat --loglevel=info
```
...
```bash
# List active tasks
celery -A full.celery inspect active

# List scheduled tasks (ETA tasks)
celery -A full.celery inspect scheduled

# List registered tasks
celery -A full.celery inspect registered

# Worker statistics
celery -A full.celery inspect stats
...
def test_periodic_tasks_registered():
    ...
    from full.celery import setup_periodic_tasks
...
export CELERY_BROKER_URL="redis://localhost:6379/0"
celery -A full.celery worker --loglevel=debug --pool=solo
```
