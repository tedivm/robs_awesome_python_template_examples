...
The CLI application is defined in `full/cli.py` and automatically configured as an entry point in `pyproject.toml`:
...
After installation, the CLI is available as the `full` command.
...
```bash
full my-command --option value argument
```
...
Add a basic command to `full/cli.py`:
...
```bash
full process input.txt --output output.txt --verbose
full process input.txt -o output.txt -v
```
...
```bash
full export --count 50 --format yaml --output data.yaml
```
...
full db migrate
full db seed
full user create john john@example.com
full user list
```
...
```python
from full.commands import database, users
...
from full.cli import app, syncify
import asyncio
