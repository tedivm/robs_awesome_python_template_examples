...
Manage application settings with `pydantic-settings` in `full/conf/settings.py` — update the existing class, never create a new one. Use `SecretStr` / `SecretBytes` for sensitive data. Optional settings default to `None`, never `""`. All fields use `Field(description=...)`.
...
Use async SQLAlchemy 2.0 with the declarative class system. Models live in `full/models/`. Use Alembic for all schema changes (migrations must work on both SQLite and PostgreSQL). Always use explicit `and_()` in queries — no implicit AND.
...
Expose user-facing commands via Typer. The main CLI entrypoint is `full/cli.py`. Use the `@syncify` decorator (from `full/cli.py`) for async commands — never use `asyncio.run()` directly. Use `Annotated[]` for all arguments and options.
...
- Database models must live in `full/models/`.
- The primary settings file must live in `full/conf/settings.py`.
