...
The caching layer is defined in `full/services/cache.py`. It provides helper functions and a `NoOpCache` fallback for when caching is disabled.
...
```python
from full.services.cache import get_cached, set_cached, delete_cached, clear_cache
