...
The CLI is registered in `pyproject.toml` as a project script pointing to `full.cli:app`.

The main app is defined in `full/cli.py`. Domain-specific subcommand groups live in separate `full/cli_<domain>.py` files and are mounted onto the main app.

---

## Async Commands — `@syncify`

Typer runs commands synchronously, but this project uses async throughout (database access, HTTP calls, etc.). Use the `@syncify` decorator from `full/cli.py` to bridge them:
...
Use `get_session` from `full.services.db` for database-backed commands. Always use it as an async context manager:
...
- [ ] New commands in `full/cli.py` or `full/cli_<domain>.py`
...
- [ ] New subapp mounted in `full/cli.py` with `app.add_typer(...)`
