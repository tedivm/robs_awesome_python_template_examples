...
The CLI application is defined in `library/cli.py` and automatically configured as an entry point in `pyproject.toml`:
...
After installation, the CLI is available as the `library` command.
...
```bash
library my-command --option value argument
```
...
Add a basic command to `library/cli.py`:
...
```bash
library process input.txt --output output.txt --verbose
library process input.txt -o output.txt -v
```
...
```bash
library export --count 50 --format yaml --output data.yaml
```
...
```python
from library.commands import database, users
